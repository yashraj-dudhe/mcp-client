import { createRemoteConnection } from '../connection.esnext';
import { ROOT_ID, NODE_TYPE_ROOT, UPDATE_PROPERTY_TYPE_EVENT_LISTENER, UPDATE_PROPERTY_TYPE_ATTRIBUTE, UPDATE_PROPERTY_TYPE_PROPERTY, NODE_TYPE_ELEMENT, NODE_TYPE_COMMENT, NODE_TYPE_TEXT } from '../constants.esnext';
...
class RemoteReceiver {
   ...
   */
  connection;
  attached = /* @__PURE__ */ new Map([[ROOT_ID, this.root]]);
  ...
  constructor({
    ...
    function runSubscribers(attached2) {
      const subscribed = subscribers.get(attached2.id);
      if (subscribed) {
        for (const subscriber of subscribed) {
          subscriber(attached2);
        }
      }
    }
  ...
  subscribe({ id }, subscriber, {
    ...
    if (subscribersSet == null) {
      subscribersSet = /* @__PURE__ */ new Set();
