...
class DOMRemoteReceiver {
  ...
  constructor({
    ...
    this.connection = connection.createRemoteConnection({
      call: (id, method, ...args) => {
        const element = id === constants.ROOT_ID && this.root.nodeType !== 11 ? this.root : attached.get(id);
        return call ? call(element, method, ...args) : element[method](...args);
      },
      insertChild: (id, child, index) => {
        const parent = id === constants.ROOT_ID ? this.root : attached.get(id);
        ...
        case constants.NODE_TYPE_ELEMENT:
          {
            ...
            if (node.properties) {
              ...
              for (const property of Object.keys(node.properties)) {
                ...
                retain?.(value);
                updateRemoteProperty(normalizedChild, property, value, constants.UPDATE_PROPERTY_TYPE_PROPERTY);
              }
            } else {
            ...
            if (node.attributes) {
              for (const attribute of Object.keys(node.attributes)) {
                ...
                retain?.(value);
                updateRemoteProperty(normalizedChild, attribute, value, constants.UPDATE_PROPERTY_TYPE_ATTRIBUTE);
              }
            }
            ...
            if (node.eventListeners) {
              for (const event of Object.keys(node.eventListeners)) {
                ...
                retain?.(listener);
                updateRemoteProperty(normalizedChild, event, listener, constants.UPDATE_PROPERTY_TYPE_EVENT_LISTENER);
              }
            }
...
exports.DOMRemoteReceiver = DOMRemoteReceiver;
